...
import './OrderForSeller.css';
import Modal from 'react-modal';

const { Option } = Select;
Modal.setAppElement('#root');

export const OrderForSeller = () => {
  const { id } = useParams();
    ...
    try {
      ...
      if (token) {
        ...
        const response = await axios.get(
          `PurchaseTicket/getListPurchaseTicketForSeller/5/${page}/${status}`
        );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        const statusStyle = {
          0: { color: 'black', fontWeight: 'bold' },
          1: { color: 'red', fontWeight: 'bold' },
          2: { color: 'green', fontWeight: 'bold' },
        };
